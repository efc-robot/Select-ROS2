...
void usage()
{
    ...
    std::cout << "DynamicHelloWorld [role] [-c mycount] [-s sleep]" << std::endl;
    std::cout << "role: Can be 'publisher' (default) or 'subscriber'." << std::endl;
    std::cout << "mycount: Number of samples sent by a publisher. Without effect on subcribers." << std::endl;
    ...
    try
    {
        for (int i = 1; i < argc; ++i)
        {
            ...
            else if(strcmp(argv[i],"-c")==0)
            {
                if (argc <= i+1)
                {
                    std::cout << "mycount expected" << std::endl;
                    Log::Reset();
                    return false;
                }
                mycount = atoi(argv[++i]);
