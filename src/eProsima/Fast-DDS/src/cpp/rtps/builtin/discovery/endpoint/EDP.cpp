        ...
        const ReaderQos& rqos)
{
        ...
        const ParticipantProxyData& participant_data)
            {
                ...
                {
                    rpd->set_locators(participant_data.default_locators);//在这里存储了含有ip+port的locator
                }
                else
                {
    ...
    GUID_t participant_guid;//是否在这里引入了topicName，然后进行发送
    ReaderProxyData* reader_data = this->mp_PDP->addReaderProxyData(reader->getGuid(), participant_guid, init_fun);//此处init_fun的含义我认为是函数里面的rpd就是这里的reader_data
    reader_data->m_topicKey = att.topic_key;
