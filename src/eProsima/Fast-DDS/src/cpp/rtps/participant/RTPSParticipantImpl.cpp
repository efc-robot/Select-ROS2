...
namespace eprosima
{
    namespace fastrtps
    {
        namespace rtps
        {
            ...
            {
                return (reader == c_EntityId_SPDPReader) ? c_EntityId_SPDPWriter : (reader == c_EntityId_SEDPPubReader)  ? c_EntityId_SEDPPubWriter
            ...
            RTPSParticipantImpl::RTPSParticipantImpl(
                ...
                : domain_id_(domain_id), m_att(PParam), m_guid(guidP, c_EntityId_RTPSParticipant), m_persistence_guid(persistence_guid, c_EntityId_RTPSParticipant), mp_builtinProtocols(nullptr), mp_ResourceSemaphore(new Semaphore(0)), IdCounter(0), type_check_fn_(nullptr)
#if HAVE_SECURITY
                  ,
                  m_security_manager(this)
#endif // if HAVE_SECURITY
                  ,
                  mp_participantListener(plisten), mp_userParticipant(par), mp_mutex(new std::recursive_mutex()), is_intraprocess_only_(should_be_intraprocess_only(PParam)), has_shm_transport_(false)
            {
            ...
            bool RTPSParticipantImpl::create_reader(
                ...
                bool enable,
                const Functor &callback,
                string_255 topic_key)
            {
                std::string type = (param.endpoint.reliabilityKind == RELIABLE) ? "RELIABLE" : "BEST_EFFORT";
                logInfo(RTPS_PARTICIPANT, "Creating reader of type " << type);
                EntityId_t entId;
                //在这里完成了entityID的赋值，在这里也可以完成topic_key的赋值
            ...
            bool RTPSParticipantImpl::createWriter(
                ...
                bool isBuiltin)
            {
                auto callback = [hist, listen, this](const GUID_t &guid, WriterAttributes &param, IPersistenceService *persistence,
                ...
                bool isBuiltin)
            {
                ...
                auto callback = [hist, listen, &payload_pool, this](const GUID_t &guid, WriterAttributes &param, IPersistenceService *persistence,
                                                                    bool is_reliable) -> RTPSWriter *
                {
                    if (is_reliable)
                    {
            ...
            bool RTPSParticipantImpl::createReader(
                ...
                bool enable)
            {
                auto callback = [hist, listen, this](const GUID_t &guid, ReaderAttributes &param, IPersistenceService *persistence,
                ...
                bool enable)
            {
                ...
                auto callback = [hist, listen, &payload_pool, this](const GUID_t &guid, ReaderAttributes &param, IPersistenceService *persistence,
                                                                    bool is_reliable) -> RTPSReader *
                {
                    if (is_reliable)
                    {
                ...
                return create_reader(ReaderOut, param, entityId, isBuiltin, enable, callback, topic_key);
            }
                ...
                Endpoint *pend)
            {
                ...
                for (auto it = pend->m_att.remoteLocatorList.begin(); it != pend->m_att.remoteLocatorList.end(); ++it)
                {
                    ...
                    {
                        logWarning(RTPS_PARTICIPANT, "Cannot create send resource for endpoint remote locator (" << pend->getGuid() << ", " << (*it) << ")");
                    }
                }
            ...
            {
                if (m_att.builtin.metatrafficMulticastLocatorList == MulticastLocatorList && m_att.builtin.metatrafficUnicastLocatorList == UnicastLocatorList)
                {
                    // no mutation
                    return false;
                }
                ...
                struct ResetLogical
                {
                    ...
                    ResetLogical(
                        const Transports &tp)
                        : Transports_(tp), tcp4(nullptr), tcp6(nullptr)
                    {
