    ...
    namespace fastrtps
    {
        namespace rtps
        {
            ...
            StatefulWriter::StatefulWriter(
                ...
                : RTPSWriter(pimpl, guid, att, payload_pool, change_pool, hist, listen), periodic_hb_event_(nullptr), nack_response_event_(nullptr), ack_event_(nullptr), m_heartbeatCount(0), m_times(att.times), matched_readers_(att.matched_readers_allocation), matched_readers_pool_(att.matched_readers_allocation), next_all_acked_notify_sequence_(0, 1), all_acked_(false), may_remove_change_cond_(), may_remove_change_(0), disable_heartbeat_piggyback_(att.disable_heartbeat_piggyback), disable_positive_acks_(att.disable_positive_acks), keep_duration_us_(att.keep_duration.to_ns() * 1e-3), last_sequence_number_(), biggest_removed_sequence_number_(), sendBufferSize_(pimpl->get_min_network_send_buffer_size()), currentUsageSendBufferSize_(static_cast<int32_t>(pimpl->get_min_network_send_buffer_size())), m_controllers()
            {
                init(pimpl, att);
            }
                ...
                const ReaderProxyData &rdata)
            {
                ...
                if (matched_readers_pool_.empty())
                {
                    ...
                    {
                        logWarning(RTPS_WRITER, "Maximum number of reader proxies (" << max_readers << ") reached for writer " << m_guid);
                        return false;
                    }
                }
                else
                ...
                temp_topic_key = rdata.m_topicKey;
                if (temp_topic_key.length() == 13)
                {
                    std::cout << "OK" << std::endl;
                }
                rp->get_guid().set_instance_id(temp_topic_key);
                ...
                if (min_seq != SequenceNumber_t::unknown())
                {
                    ...
                    if (next_all_acked_notify_sequence_ <= min_low_mark)
                    {
                        if ((mp_listener != nullptr) && (min_low_mark >= get_seq_num_min()))
                        {
                            ...
                            SequenceNumber_t seq{};
                            SequenceNumber_t end_seq = min_seq > next_all_acked_notify_sequence_ ? min_seq : next_all_acked_notify_sequence_;
                ...
                std::unique_lock<RecursiveTimedMutex> &lock)
            {
                ...
                SequenceNumber_t calc = min_low_mark < get_seq_num_min() ? SequenceNumber_t() : (min_low_mark - get_seq_num_min()) + 1;
