...
rcl_init_options_fini(rcl_init_options_t * init_options)
{
  ...
  RCL_CHECK_ALLOCATOR(&allocator, return RCL_RET_INVALID_ARGUMENT);
  rmw_ret_t rmw_ret = rmw_init_options_fini(&(init_options->impl->rmw_init_options));//rmw_ret只是用来判断init_options是否正确的标志而已
  if (RMW_RET_OK != rmw_ret) {
    RCL_SET_ERROR_MSG(rmw_get_error_string().str);
    return rcl_convert_rmw_ret_to_rcl_ret(rmw_ret);
  }
  allocator.deallocate(init_options->impl, allocator.state);//deallocate 是将分配出的空间(没有被使用的空间) 放回主存, 此句话运行完成后，init_options->impl被还原为空或初始状态
  return RCL_RET_OK;
}
