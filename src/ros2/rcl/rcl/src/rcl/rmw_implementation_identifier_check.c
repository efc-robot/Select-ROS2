...
rcl_ret_t rcl_rmw_implementation_identifier_check(void)
{
  ...
  if (expected_rmw_impl) {
    ...
    // 如果export了不存在的dds或者未安装的dds，则会返回错误
    // 此时 expected_rmw_impl为rmw_cyclone, 是无法识别的dds
    const rcutils_error_string_t rmw_error_msg = rcl_get_error_string();
    //expected_rmw_impl 和 actual_rmw_impl_id 是我们使用的rmw种类，当export RMW_IMPLEMENTATION=rmw_cyclonedds时，它们的值是"rmw_cyclonedds"
    //可通过rmw_error_msg.str看到错误信息， 比如export错误的dds后，给出的错误信息是：failed to find sharedlibrary\rmw_cyclone\, at /home/asamu/ros2/foxy_debug/src/ros2/rmw_implementation/rmw_implementation/src/functions.cpp:73
    //如果export了不存在的dds或者未安装的dds，则会返回错误，会结束此程序，如若未指定dds，就会指定为默认的fastrtps
    rcl_reset_error();
