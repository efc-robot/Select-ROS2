...
TEST_F(NodeGraphMultiNodeFixture, test_node_info_subscriptions)
{
  ...
  char* topic_key = "55";
  ret = rcl_subscription_init(&sub, this->node_ptr, ts, this->topic_name.c_str(),topic_key, &sub_ops);
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  rcl_reset_error();
  ...
  ret =
    rcl_subscription_init(&sub2, this->remote_node_ptr, ts, this->topic_name.c_str(), topic_key, &sub_ops2);
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  rcl_reset_error();
...
TEST_F(CLASSNAME(TestGraphFixture, RMW_IMPLEMENTATION), test_graph_query_functions)
{
  ...
  rcl_subscription_options_t sub_ops = rcl_subscription_get_default_options();
  char* topic_key = "55";
  ret = rcl_subscription_init(&sub, this->node_ptr, ts, topic_name.c_str(), topic_key,&sub_ops);
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  rcl_reset_error();
  ...
  std::thread topic_thread(
    [this, &topic_changes_promise]() {
      ...
      ret = rcl_subscription_init(
        ...
        "/chatter_test_graph_guard_condition_topics",topic_key, &sub_ops);
      EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
      // sleep
      std::this_thread::sleep_for(std::chrono::milliseconds(200));
