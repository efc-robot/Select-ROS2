...
public:
  ...
  void SetUp() override
  {
    ...
    char* topic_key = "55";
    ret = rcl_subscription_init(&subscription, this->node_ptr, ts, topic, topic_key, &subscription_options);
    ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  }
...
TEST_F(CLASSNAME(TestSubscriptionFixture, RMW_IMPLEMENTATION), test_subscription_bad_init) {
  ...
  EXPECT_EQ(
    ...
    rcl_subscription_init(&subscription, nullptr, ts, topic, topic_key, &subscription_options));
  rcl_reset_error();

  EXPECT_EQ(
    RCL_RET_NODE_INVALID,
    rcl_subscription_init(&subscription, &invalid_node, ts, topic,topic_key, &subscription_options));
  rcl_reset_error();

  rcl_ret_t ret = rcl_subscription_init(
    &subscription, this->node_ptr, ts, "spaced name",topic_key, &subscription_options);
  EXPECT_EQ(RCL_RET_TOPIC_NAME_INVALID, ret) << rcl_get_error_string().str;
  rcl_reset_error();
  ret = rcl_subscription_init(
    &subscription, this->node_ptr, ts, "sub{ros_not_match}",topic_key, &subscription_options);
  EXPECT_EQ(RCL_RET_TOPIC_NAME_INVALID, ret) << rcl_get_error_string().str;
  rcl_reset_error();
...
TEST_F(CLASSNAME(TestSubscriptionFixture, RMW_IMPLEMENTATION), test_bad_take_loaned_message) {
  ...
  rmw_ret_t ret = rcl_subscription_init(
    &subscription, this->node_ptr, ts, topic,topic_key,
...
TEST_F(CLASSNAME(TestSubscriptionFixture, RMW_IMPLEMENTATION), test_init_fini_maybe_fail)
{
  ...
  {
    rcl_ret_t ret = rcl_subscription_init(
      &subscription, this->node_ptr, ts, topic, topic_key, &subscription_options);
