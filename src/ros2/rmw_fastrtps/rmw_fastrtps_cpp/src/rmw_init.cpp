...
{
  //在控制调试台中输入init_options.implemention_identifier, 可以查看到使用的rmw种类 //exprot RMW_IMPLEMENTATION=rmw_connextdds 之后， 上述为0x0, 但是在经过搜索之后，在控制调试台中输入init_options.implemention_identifier，发现值为rmw_fastrtps_cpp, 猜想是没有找到rmw_connextdds，于是使用了默认的rmw_fastrtps_cpp
...
rmw_ret_t
rmw_init(const rmw_init_options_t * options, rmw_context_t * context)//经过这个函数后，空的context得到赋值
{
