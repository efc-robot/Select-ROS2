...
namespace rmw_fastrtps_cpp
{
      ...
      bool create_subscription_listener)
  {
          ...
          if (info->subscriber_)
          {
            if (!Domain::removeSubscriber(info->subscriber_))
            {
              RMW_SAFE_FWRITE_TO_STDERR(
                  "Failed to remove subscriber after '" RCUTILS_STRINGIFY(__function__) "' failed.\n");
            }
          }
    ...
    subscriberParam.topic.topic_key = topic_key;
    ...
    std::cout << "topic_key" << topic_key << std::endl;
    info->subscriber_ = Domain::createSubscriber(participant, subscriberParam, info->listener_, topic_key);
    if (!info->subscriber_)
    {
