...
load_library()
{
  ...
  //如若没有设置RMW_IMPLEMENTATION，则使用默认的RMW_IMPLEMENTATION, env_var为空（size==0）
  if (env_var.empty()) {
...
//library_path = "install/lib/librmw_fastrtps_cpp.so";
...
// 在我load的库里面去找有没有这个库 如果有就返回这个库的地址
// lib中找库的操作写在src/ros2/rcpputils/src/shared_library.cpp中，lib是install/lib中的dds库ros2_foxy_debug/install/lib/librmw_cyclonedds_cpp.so"
{
  if (!lib) {
    if (!rmw_error_is_set()) {
...
//symbol_name = "rmw_init_options_init"
void *
...
RMW_INTERFACE_FN(
  ...
  6, ARG_TYPES(
    const rmw_node_t *, const rosidl_message_type_support_t *, const char *,
    const rmw_qos_profile_t *, const rmw_subscription_options_t *, const char *))
...
void prefetch_symbols(void)
{
  ...
  // symbol_rmw_get_implementation_identifier = get_symbol(rmw_get_implementation_identifier)
